<nav x-data="{ open: false }" class="absolute top-0 w-full left-0 z-50">
    <!-- Primary Navigation Menu -->
    <div class="max-w-9xl w-full mx-auto px-4 sm:px-6 lg:px-8 bg-gray-100 rounded-md mt-10">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    <x-nav-link href="https://snowball-lang.gitbook.io/docs/">
                        {{ __('Documentation') }}
                    </x-nav-link>
                </div>
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    <x-nav-link :href="route('pkgs')" :active="request()->routeIs('pkgs')">
                        {{ __('Packages') }}
                    </x-nav-link>
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm text-gray-700 font-bold">Log in</a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="ml-4 text-sm text-gray-700 font-bold">Register</a>
                    @endif
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        @auth
            <div class="pt-2 pb-3 space-y-1">
                <x-responsive-nav-link href="https://snowball-lang.gitbook.io/docs/">
                    {{ __('Documentation') }}
